...
def processEventalignFromStdout(options, eventalign, refs, genome, modified, alphabet):

    headers = eventalign.stdout.next().split('\t')
    idx={}
    idx['contig'] = headers.index("contig")
    idx['ref_pos'] = headers.index("position")
    idx['ref_kmer'] = headers.index("reference_kmer")
    ...
    line = eventalign.stdout.next().split('\t')
    kmer=len(line[idx['ref_kmer']])
    # TODO: is there a better way than throwing away the first line?
...
def embedEventalign(options, fasta, eventalign, reads, outPrefix, modified):
    ...
    trainData, premadeFilenames = processEventalignFromStdout(options, eventalign, refs, genome, modified, alphabet)
